...
from carbon.carboncalculation import (
    calculate_final_emissions,
    calculate_process_emissions, calculate_product_emissions
)
from carbondata.carbondata import (
    component_factors_A, component_factors_C, component_factors_D,
    fastener_factors_A,
    fastener_factors_C,
    fastener_factors_D,
    machine_factors
)
from carbondata.find_carbondata import (
    find_product_factors_A, find_product_factors_C,
    find_product_factors_D
)
from models.results import DSSGNode, EmissionResult


def find_dssg_single_target(t: int, d: int, dg, dg_products, dg_quantities, cc_matrix, cf_matrix,
               mc_matrix, mf_matrix, pc_matrix) -> tuple[dict[int, DSSGNode], nx.DiGraph, list[int], EmissionResult]:
    ...
    dssg[t] = DSSGNode(type='c', d_dir=d, parent=None, blocked_components=None)
    queue.append(t)
    G = nx.DiGraph()
    ...
    while queue:
        ...
        p_type = dssg[p].type
        dssg_list.append(p)
        product_material = dg_products[p]
        if p_type == 'c':
            product_factors_A = find_product_factors_A(product_material, fastener_factors_A, component_factors_A)
            product_factors_C = find_product_factors_C(product_material, fastener_factors_C, component_factors_C)
            product_factors_D = find_product_factors_D(product_material, fastener_factors_D, component_factors_D)
            product_result = calculate_product_emissions(p, p_type, product_material, dg_quantities, product_factors_A, product_factors_C, product_factors_D)
            product_emissions.append(product_result)
        if p_type == 'f':
            product_factors_A = find_product_factors_A(product_material, fastener_factors_A, component_factors_A)
            product_factors_C = find_product_factors_C(product_material, fastener_factors_C, component_factors_C)
            product_factors_D = find_product_factors_D(product_material, fastener_factors_D, component_factors_D)
            product_result = calculate_product_emissions(p, p_type,
                                                         product_material,
                                                         dg_quantities,
                                                         product_factors_A,
                                                         product_factors_C,
                                                         product_factors_D)
            machine_result = calculate_process_emissions(product_material, machine_factors)
            product_emissions.append(product_result)
            machine_emissions.append(machine_result)
    ...
    final_emissions = calculate_final_emissions(product_emissions, machine_emissions, t, d)

    return dssg, G, dssg_list, final_emissions