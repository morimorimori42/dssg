""" function to find disassembly sequence structure graphs (dssg) """
from collections import deque
import networkx as nx


def find_dssg(t, d_dir, dg, cc_matrix, cf_matrix, mc_matrix, mf_matrix, pc_matrix):
    """ function to create the dssg """
    dssg = {}
    queue = deque()

    # Start process, select target component out of DG, choose disassembly direction
    dssg[t] = {'type': 'c', 'd_dir': d_dir, 'parent': None}
    queue.append(t)
    ...
    while queue:
        ...
        G.add_node(p, label=str(p), **dssg[p])
            ...
            if cc_cell and cc_cell.components:
                for c_prime in cc_cell.components:
                    ...
                    if c_prime in dg:
                        ...
                        if any(mc_cell and mc_cell.components for mc_cell in
                               mc_row[:d_dir - 1]) or \
                                any(mc_cell and mc_cell.components for mc_cell
                                    in mc_row[d_dir:]):
                            continue
                        ...
                        dssg[c_prime] = {'type': 'c', 'd_dir': d_dir_prime, 'parent': p}
            ...
            if cc_cell and cc_cell.fasteners:
                for f_prime in cc_cell.fasteners:
                    ...
                    if cf_entry:
                        ...
                        dssg[f_prime] = {'type': 'f', 'd_dir': d_dir_prime, 'parent': p}
                    ...
                    for f_prime in cc_cell.fasteners:
                        ...
                        if cf_entry:
                            ...
                            for j in range(len(cc_matrix.data)):
                                ...
                                d_dir_prime = cf_entry.direction
                                dssg[f_prime] = {
                                    'type': 'f', 'd_dir': d_dir_prime, 'parent': p
                                }
                                queue.append(f_prime)
                                del dg[index]
            ...
            if cf_entry:
                ...
                if mf_cell and mf_cell.component:
                    ...
                    if c_prime in dg:
                        ...
                        d_dir_prime = d_dir  # Assuming the same disassembly direction for the connected component
                        dssg[c_prime] = {
                            'type': 'c', 'd_dir': d_dir_prime, 'parent': c_prime
                        }
                        queue.append(c_prime)
                        del dg[index]
    ...
    for p in dssg:
        ...
        if p_type == 'c':
            ...
            if pc_entries:
                ...
                dssg[p]['blocking_count'] = pc_entry.blocking_components
